# aws-etl-data-pipeline
End-to-end ETL pipeline built on AWS (S3 → Glue → Redshift) with a predictive model in SageMaker Canvas. Includes data ingestion, cleaning, transformation, orchestration, and a short video walkthrough.

## 📄 Project Materials

- **[Final Project Presentation (PDF)](https://github.com/meesumkazmi1/aws-etl-data-pipeline/blob/main/docs/Presenation.pdf)**
- **[Project Description Document](https://github.com/meesumkazmi1/aws-etl-data-pipeline/blob/main/docs/Description.docx)**

## 🎥 Video Walkthrough

A short walkthrough of the AWS pipeline and the SageMaker bankruptcy prediction model:

➡️ **[Watch the Video Demo](https://drive.google.com/file/d/1CP9a7d9R3zIdjIm-a42jtDOizfWFxsqc/view?usp=sharing)**

This video covers:
- S3 data lake structure  
- AWS Glue ETL transformation  
- Redshift warehouse loading  
- SageMaker Canvas ML model  
- Final investment recommendations  
